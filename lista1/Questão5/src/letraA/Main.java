...
public class Main {
	...
	public static void main(String[] args) {
		...
		try {
			FileWriter filewriter = new FileWriter(new File("data_"+args[0]+"_threads.csv"));
			String EOF = System.lineSeparator();
			filewriter.append("estrutura,num_threads,operacao,tempo"+EOF);
			for (Long time : readChannel1.recieve()) {
				filewriter.append(String.format("ConcurrentHashMap,%s,read,%d%n", args[0], time));
			}
			for (Long time : writeChannel1.recieve()) {
				filewriter.append(String.format("ConcurrentHashMap,%s,write,%d%n", args[0], time));
			}
			for (Long time : readChannel2.recieve()) {
				filewriter.append(String.format("SynchronizedMap,%s,read,%d%n", args[0], time));
			}
			for (Long time : writeChannel2.recieve()) {
				filewriter.append(String.format("SynchronizedMap,%s,write,%d%n", args[0], time));
			}
			filewriter.close();
