...
public class Main {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < threadsADD1.length; i++) {
			threadsADD1[i] = new ThreadAdd(map1, writeChannel1, op_threads_escrita[i]);
			threads1[i] = new Thread(threadsADD1[i]);
		}
		for (int i = 0; i < threadsGET1.length; i++) {
			threadsGET1[i] = new ThreadGet(map1, readChannel1, op_threads_leitura[i]);
		...
		for (int i = 0; i < threadsADD2.length; i++) {
			threadsADD2[i] = new ThreadAdd(synchMap, writeChannel2, op_threads_escrita[i]);
			threads2[i] = new Thread(threadsADD2[i]);
		}
		for (int i = 0; i < threadsGET2.length; i++) {
			threadsGET2[i] = new ThreadGet(synchMap, readChannel2, op_threads_leitura[i]);
		...
		try {
			...
			for (Long time : readChannel1.recieve()) {
				filewriter.append(String.format("ConcurrentHashMap,%d,%d,%s,read,%d%n", num_threads, op_total, proporcao, time));
			}
			for (Long time : writeChannel1.recieve()) {
				filewriter.append(String.format("ConcurrentHashMap,%d,%d,%s,write,%d%n", num_threads, op_total, proporcao, time));
			}
			for (Long time : readChannel2.recieve()) {
				filewriter.append(String.format("SynchronizedMap,%d,%d,%s,read,%d%n", num_threads, op_total, proporcao, time));
			}
			for (Long time : writeChannel2.recieve()) {
				filewriter.append(String.format("SynchronizedMap,%d,%d,%s,write,%d%n", num_threads, op_total, proporcao, time));
			}
			filewriter.close();
